...
export class AuditServiceProxy {
     ...
     * @return Success
     */
    getAll(maxResultCount: number | null | undefined, skipCount: number | null | undefined): Observable<PagedResultDtoOfAuditDto> {
        let url_ = this.baseUrl + "/api/services/app/Audit/GetAll?";
        ...
        return this.http.request("get", url_, options_).pipe(_observableMergeMap((response_ : any) => {
            ...
            if (response_ instanceof HttpResponseBase) {
                try {
                    ...
                    return <Observable<PagedResultDtoOfAuditDto>><any>_observableThrow(e);
                }
            } else
                return <Observable<PagedResultDtoOfAuditDto>><any>_observableThrow(response_);
        }));
    }

    protected processGetAll(response: HttpResponseBase): Observable<PagedResultDtoOfAuditDto> {
        const status = response.status;
        const responseBlob = 
            response instanceof HttpResponse ? response.body : 
            (<any>response).error instanceof Blob ? (<any>response).error : undefined;
        ...
        if (status === 200) {
            ...
            let result200: any = null;
            let resultData200 = _responseText === "" ? null : JSON.parse(_responseText, this.jsonParseReviver);
            result200 = PagedResultDtoOfAuditDto.fromJS(resultData200);
            return _observableOf(result200);
            }));
        } else if (status !== 200 && status !== 204) {
            return blobToText(responseBlob).pipe(_observableMergeMap(_responseText => {
        ...
        return _observableOf<PagedResultDtoOfAuditDto>(<any>null);
    }
}
...
export class PagedResultDtoOfAuditDto implements IPagedResultDtoOfAuditDto {
    totalCount: number | undefined;
    items: AuditDto[] | undefined;

    constructor(data?: IPagedResultDtoOfAuditDto) {
        if (data) {
            for (var property in data) {
                if (data.hasOwnProperty(property))
                    (<any>this)[property] = (<any>data)[property];
        ...
        if (_data) {
            ...
            if (Array.isArray(_data["items"])) {
                ...
                for (let item of _data["items"])
                    this.items.push(AuditDto.fromJS(item));
            }
        }
    }

    static fromJS(data: any): PagedResultDtoOfAuditDto {
        data = typeof data === 'object' ? data : {};
        let result = new PagedResultDtoOfAuditDto();
        result.init(data);
        return result;
    }
    ...
    clone(): PagedResultDtoOfAuditDto {
        const json = this.toJSON();
        let result = new PagedResultDtoOfAuditDto();
        result.init(json);
        return result;
...
export interface IPagedResultDtoOfAuditDto {
    totalCount: number | undefined;
    items: AuditDto[] | undefined;
}
